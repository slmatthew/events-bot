...
class UsersTable extends Database {
	public function create(int $id) {
		$r = mysqli_query($this->link, sprintf("INSERT INTO users VALUES (%d, 0, 0, 0)", $id));
		if(!$r) throw new DatabaseException("UTC: ".mysqli_error($this->link));

		return true;
	}

	public function getById(int $id) {
		$r = mysqli_query($this->link, $this->format("SELECT * FROM users WHERE id = %d", $id));
		if(!$r) throw new DatabaseException("UTGI: ".mysqli_error($this->link));

		return mysqli_fetch_assoc($r);
	}

	public function plusReport(int $id) {
		return mysqli_query($this->link, sprintf("UPDATE users SET reports = reports + 1 WHERE id = %d", $id));
	}

	public function setModerator(int $id, int $status = 1) {
		return mysqli_query($this->link, sprintf("UPDATE users SET moderator = %d WHERE id = %d", $status, $id));
	}

	public function setStep(int $id, int $step) {
		return mysqli_query($this->link, sprintf("UPDATE users SET step = %d WHERE id = %d", $step, $id));
	}
}
